	...
	var DialogType = mobileLibrary.DialogType;
        ...
        handleSelection:function(oEvent){
           ...
           that.uniqueName.push({
                ...
                child:that.loadArray.filter(a=>a.CHAR_NUM === selectedItem[i].getBindingContext().getObject().CHAR_NUM)
                });
      ...
      // Update the input value with the cleaned numeric value
        ...
        onCharSave:function(){
            ...
            for(var i=0;i<vBoxItems.length;i++){
                ...
                // if(that.uniqueName[i].CHAR_VALUE === that.inputArray[i].getParent().getLabel().getText()){
                ...
                if (!that._onCreate) {
                  ...
                  for(var i=0;i<distinctItems.length;i++){
                    that.uniqueName.push({
                         ...
                         child:that.loadArray.filter(a=>a.CHAR_NUM === distinctItems[i].CHAR_NUM)
                         });
                         that.uniqueName[i].child.sort((a, b) => {  const charValueA = a.CHAR_VALUE; const charValueB = b.CHAR_VALUE;    
